...
class ElementHandle extends JSHandle {
  ...
  async uploadFile(...filePaths) {
    ...
    const promises = filePaths.map(filePath => (Buffer.isBuffer(filePath.localBuffer) || Buffer.isBuffer(filePath)) ? (filePath.localBuffer || filePath) : readFileAsync(filePath));
    const files = [];
    for (let i = 0; i < filePaths.length; i++) {
      const buffer = await promises[i];
      const filePath = (helper.isString(filePaths[i].filePath) || Buffer.isBuffer(filePaths[i])) ? (filePaths[i].filePath || `buffer-${i}.${(await fileType.fromBuffer(buffer)).ext}`) : filePaths[i];
      const file = {
        name: filePath,
